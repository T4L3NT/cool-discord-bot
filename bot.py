...
#######################################################################################################################
    ...
    async def cmd_join(self, channel, server, author, voice_channel):
        ...
        if not author.voice_channel:
            raise exceptions.CommandError(self.str.get('cmd-join-novc', 'You are not in a voice channel!'))
        ...
        if not chperms.connect:
            ...
            raise exceptions.CommandError(
                self.str.get('cmd-join-noperms-connect', "Cannot join channel `{0}`, no permission to connect.").format(author.voice_channel.name),
                expire_in=25
            )
            ...
            raise exceptions.CommandError(
                self.str.get('cmd-join-noperms-speak', "Cannot join channel `{0}`, no permission to speak.").format(author.voice_channel.name),
                expire_in=25
            )
        ...
        return Response(self.str.get('cmd-join-reply', 'Connected to `{0.name}`').format(author.voice_channel))
